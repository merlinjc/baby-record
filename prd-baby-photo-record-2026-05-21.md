...
### 7.5 Profile页优化方向（P1补强）

#### 用户Profile优化

1. **从“入口页”升级为“个人工作台”**
   - 现状问题：当前个人页偏工具导航，缺少用户与当前家庭协作状态的即时反馈
   - 优化方向：首屏同时展示当前身份、当前档案、协作人数、最近上传动作、待处理事项
   - 价值：降低“我现在能做什么”的理解成本

2. **增加“当前家庭上下文”模块**
   - 展示项：当前宝宝、当前角色、是否可上传、可否管理成员、最近一次切换时间
   - 交互：一键跳转到宝宝档案页切换当前档案
   - 价值：避免用户在多宝宝或被邀请场景下误操作到错误档案

3. **增加“协作状态提醒”模块**
   - 展示项：家庭成员数、待接受邀请数、最近加入成员、最近分享行为
   - 价值：强化产品“家庭共享”差异化定位，而不仅是单人相册工具

4. **增加“安全与数据状态”模块**
   - 展示项：最近上传成功时间、回收站照片数、当前权限策略说明
   - 价值：增强用户对照片资产安全性的感知

5. **增加“快捷任务”而非仅快捷入口**
   - 任务示例：继续上传今天照片、查看本月时间轴、处理回收站、邀请家人加入
   - 价值：从静态导航改为任务驱动，提升活跃和转化

6. **补齐“完善个人资料”闭环**
   - 现状问题：当前用户首次进入小程序后会自动建用户，但昵称可能为空，邀请时容易以默认身份对外展示
   - 优化方向：个人页增加资料完成态卡片、独立资料编辑页、邀请前置校验
   - 展示项：昵称是否已填写、头像是否已设置、资料未完成时的风险提示
   - 价值：降低家庭协作中的身份误解和默认资料传播风险

7. **邀请机制升级为一次性邀请码**
   - 现状问题：纯链接分享容易被转发扩散，邀请码被多人重复使用会带来越权加入风险
   - 优化方向：由管理员生成 8 位一次性邀请码，加入成功后立即失效
   - 展示项：邀请码、是否一次性、过期提醒、已使用失效提醒
   - 价值：让家庭分享更可控，避免邀请链接被放大传播

#### 宝宝Profile优化

1. **从“列表页”升级为“档案管理页”**
   - 现状问题：当前仅能看列表和切换，缺少每个档案的状态概览
   - 优化方向：每个档案卡片展示年龄、角色、照片数、最近拍摄日期、成员数
   - 价值：帮助用户快速识别该档案是否仍在活跃使用

2. **明确“当前档案”的业务影响范围**
   - 展示说明：当前档案决定上传归属、时间轴默认数据源、家庭成员管理对象
   - 价值：避免用户误切换后造成内容写入错误

3. **增加“我的角色”展示**
   - 展示项：创建者、管理员、普通成员、仅查看者
   - 价值：让用户在进入档案页时立即理解自己能否上传、删除、邀请、恢复

4. **增加“档案健康度/活跃状态”**
   - 展示项：最近上传时间、近30天新增照片数、是否有地点标签、是否有家庭成员
   - 价值：利于用户发现空档案、停用档案和需要补充运营的档案

5. **补齐档案管理动作**
   - P1建议包含：切换当前档案、查看档案详情、查看成员入口、进入时间轴、进入上传
   - P2可扩展：编辑资料、封面图、标签、归档状态

#### 验收标准补充

- 用户进入“我的”页后，3秒内可识别当前宝宝、当前角色和下一步高频动作
- 用户进入“宝宝档案”页后，5秒内可判断每个档案是否活跃、自己具备何种权限
- 用户未填写昵称时，进入“我的”页后应明确看到资料未完成提示，并可一跳进入资料编辑页
- 用户资料未完成时，发起家庭邀请应被拦截，并提示先完善个人资料
- 一次性邀请码在首个被邀请人加入后必须立即失效，再次使用时应提示已失效或已被使用
- 回收站中不应展示当前用户无恢复权限的照片
- 多宝宝场景下，误将照片上传到错误宝宝档案的比例明显下降
